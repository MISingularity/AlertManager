...
func (c *Config) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	for _, rcv := range c.Receivers {
		...
		for _, YunPianSMS := range rcv.YunPianConfigs {
			if YunPianSMS.apikey == "" {
				if c.Global.YunPianSMSapikey == "" {
					return fmt.Errorf("no global YunPianSMS apikey set")
				}
...
type YunPianSMStplvalue struct {
	alert_type Strings
	service    Strings
	reason     Strings
}
...
type Receiver struct {
	...
	YunPianConfigs   []*YunPianConfig   `yaml:"YunPiansms_configs,omitempty"`
