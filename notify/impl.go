...
func Build(confs []*config.Receiver, tmpl *template.Template) map[string]Fanout {
	...
	for _, nc := range confs {
		...
		for i, c := range nc.YunPianSMSConfigs {
			n := NewYunPianSMS(c, tmpl)
			add(i, n, filter(n, c))
		}
...
// YunPianSMS implements a notifier for YunPianSMS notifications.
type YunPianSMS struct {
	conf *config.YunPianConfig
	tmpl *template.Template
}

// NewYunPianSMS return a new YunPianSMS notification handler.
func NewYunPianSMS(conf *config.YunPianSMSConfig, tmpl *template.Template) *YunPianSMS {
	return &YunPianSMS{
		conf: conf,
		tmpl: tmpl,
	}
...
func (*YunPianSMS) name() string { return "YunPianSMS" }

// Notify implements the Notifier interface.
func (n *YunPianSMS) Notify(ctx context.Context, as ...*types.Alert) error {
	var err error
	var (
		data     = n.tmpl.Data(receiver(ctx), groupLables(ctx), as...)
		tmplText = tmplText(n.tmpl, data, &err)
	)

	if err != nil {
		return fmt.Errorf("unexpected error to creating tmplText %v ", err.Error())
	}
	...
	resp, err := ctxhttp.PostForm(ctx, http.DefaultClient, url_send_sms, req)
